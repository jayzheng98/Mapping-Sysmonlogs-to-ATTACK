...
def fetch_all(index_term, labelled_logs):
    ...
    with open('syslog.csv', 'w', newline='', encoding='utf-8') as f:
        ...
        headers = ['_id', 'CommandLine', 'EventTime', 'LocalIP', '@timestamp', 'AccountType', 'CallTrace',
        ...
        for res in results:
            try:
                if(res['_source']['RuleName'] != '-'):
                    ...
                    if temp:
                        if temp[0] == 'Port Monitors':    # To fix a bug of our sysmon's config file
                            temp[0] = 'T1547.010'
            ...
            d[headers[0]].append(res[headers[0]])
